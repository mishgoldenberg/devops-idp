...
def get_work_items(
    ...
    # Try to query Azure DevOps using WIQL with a shared PAT, filtering by the
    # signed-in user's identity (email/username).
    try:
        query = {
        ...
        if not ADO_PAT:
            raise HTTPException(
                ...
                detail="Azure DevOps PAT is not configured on the server",
            )
