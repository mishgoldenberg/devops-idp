...
6. Save the pipeline.
7. Confirm the pipeline can access the `devops-hub-deploy` variable
   group.
8. Run the pipeline from `dev` or `main`.

The pipeline expects a self-hosted offline agent pool named:
...
- The offline agent can run `docker`, `python`, `npm`, `helm`, and `oc`.
